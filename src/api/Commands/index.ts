...
import { CommandArgument, CommandContext, CommandOption } from "@adacord/discord-types";
import { Logger } from "@utils/Logger";
import { makeCodeblock } from "@utils/text";

import { sendBotMessage } from "./commandHelpers";
import { AdacordCommand,ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: AdacordCommand[];
export const commands = {} as Record<string, AdacordCommand>;
...
export const _init = function (cmds: AdacordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: AdacordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isAdacordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | AdacordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: AdacordCommand, plugin: string) {
...
export function registerCommand<C extends AdacordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",
