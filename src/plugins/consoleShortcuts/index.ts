...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => Adacord.WebpackPatcher },
        wpInstances: { getter: () => Adacord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => Adacord.Plugins },
        plugins: { getter: () => Adacord.Plugins.plugins },
        Settings: { getter: () => Adacord.Settings },
        Api: { getter: () => Adacord.Api },
        Util: { getter: () => Adacord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("adacord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (Adacord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$adacordGetWrappedComponent) {
            return forceLoad ? value.$$adacordGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = AdacordNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;
